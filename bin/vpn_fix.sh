...
docker_interface=$(sudo -u $(logname) VBoxManage showvminfo default | grep -o -E 'vboxnet\d\d?')
if [ -z "${docker_interface}" ]; then
    ...
    current_route=$(netstat -rn | grep 192.168.99)
    if [ -z "${current_route}" ]; then
        # no route, let's add it!
        route -nv add -net 192.168.99 -interface ${docker_interface}
    else
        route -nv change -net 192.168.99 -interface ${docker_interface}
    fi
